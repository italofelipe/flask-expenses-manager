...
| CD-07 | CD | Corrigir falha de bootstrap no deploy remoto (`missing required file: scripts/ensure_tls_runtime.sh`) com fallback compatível para refs legados e ambientes já provisionados | In Progress | 95% | Médio: até validar novo run, ainda existe risco residual de `.env.prod` legado sem chaves operacionais | pending-commit | 2026-02-18 |
