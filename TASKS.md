...
| I5 | Deploy AWS | Provisionar banco no plano B (RDS PostgreSQL) e documentar critérios de fallback | Done | 100% | Baixo: RDS nao foi provisionado por budget; guardrails (Budgets/Anomaly) aplicados e plano B documentado | pending-commit | 2026-02-14 |
| I6 | Deploy AWS | Configurar deploy automático (GitHub Actions -> servidor) com rollback básico | In Progress | 60% | Medio: deploy via SSM implementado com `deploy/rollback/status` e state em `/var/lib/auraxis/deploy_state.json`; pendente OIDC/IAM no GitHub Actions | 575dcea, 4586d3b | 2026-02-13 |
...
| I9 | Deploy AWS | Runbook de operação (backup, restore, rotação de credenciais, incidentes) | Done | 100% | Baixo: runbook consolidado em docs + scripts de validacao/operacao | pending-commit | 2026-02-14 |
...
| 2026-02-14 | I5 | Guardrails de custo aplicados (AWS Budgets + Cost Anomaly Detection) com limite conservador em USD e docs de custo/Plano B | pending-commit |
| 2026-02-14 | I9 | Runbook operacional consolidado (deploy/rollback, TLS, backups/restore, observabilidade, firewall, custo) | pending-commit |
