...
| I8 | Deploy AWS | Hardening de produção (secrets, TLS, firewall, least-privilege IAM) | In Progress | 65% | Medio: IAM audit indica role não-broad (SSM + CloudWatchAgent + inline S3 backups); pendente runbook operacional (I9) e revisão de políticas (remover excesso se houver) | 5825206, 2b2459c, pending-commit | 2026-02-13 |
...
| 2026-02-13 | I8 | IAM audit: instâncias DEV/PROD usando role `auraxis-ec2-ssm-role` com políticas gerenciadas mínimas (SSM + CloudWatchAgent) e inline policy para backups S3 | pending-commit |
