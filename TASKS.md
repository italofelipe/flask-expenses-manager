...
| S1 | AWS Security | Restringir acesso e hardening de instâncias EC2 (SG, NACL, IMDSv2, SSH policy, patching baseline) | In Progress | 96% | Médio: DEV/PROD com root EBS criptografado + IMDSv2 + termination protection + IAM profile + SSM online + IP estável em PROD via Elastic IP + SSH ingress removido; pendente refinamento fino de NACL/firewall + automação de patching baseline | pending-commit | 2026-02-12 |
